...
class AddressProcessor implements \Perspective\NovaposhtaShippingGraphQl\Api\Data\ProcessorInterface
{
    /**
    ...
    public function isApplicable($cart)
    {
        ...
        $result = false;
        if (empty ($addressInformation->getShippingMethodCode())) {
            $shippingMethod = $cart->getShippingAddress()->getShippingMethod();
            if (empty($shippingMethod)) {
                return false;
            }
            [$carrier, $code] = explode('_', $shippingMethod ?? '');
            if ($carrier === 'novaposhtashipping'){
                $addressInformation->setShippingMethodCode($code);
            }
        }
        if (!in_array(
